...
- `{skill_dir}/personas/{writing_persona}.yaml`
- `{skill_dir}/learned_rules.yaml`
- `{skill_dir}/history.yaml`
- `{skill_dir}/references/exemplars/index.yaml`
...
- `{skill_dir}/references/personal_frameworks.yaml`
...
- `{skill_dir}/references/personal_enhance.yaml`
...
```bash
python3 {skill_dir}/toolkit/cli.py writer-plan "题目" --keyword 关键词1 --keyword 关键词2
```
...
```bash
python3 {skill_dir}/toolkit/cli.py writer-qc article.md
```
...
```bash
python3 {skill_dir}/toolkit/cli.py writer-log article.md --topic "原始题目"
```
...
3. 比较 `draft -> final`
4. 提炼稳定规则
5. 写入 `{skill_dir}/learned_rules.yaml`
...
```bash
python3 {skill_dir}/toolkit/cli.py writer-learn --draft draft.md --final final.md
```
...
```bash
python3 {skill_dir}/toolkit/cli.py writer-import path/to/folder
```
...
```bash
python3 {skill_dir}/toolkit/cli.py preview article.md
python3 {skill_dir}/toolkit/cli.py themes
...
本文档中 `{skill_dir}` 指本 `SKILL.md` 所在目录，也就是 `rwrite` 根目录。
