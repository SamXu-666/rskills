...
CONFIG_PATHS = [
    ...
    Path.home() / ".config" / "rwrite" / "config.yaml",
]
...
def cmd_gallery(args):
    ...
    output = args.output or "/tmp/rwrite-gallery.html"
...
def cmd_writer_import(args):
    """Import local markdown files into the personal exemplar library."""
    import subprocess
    script = Path(__file__).parent.parent / "scripts" / "personal_writer.py"
    cmd = [sys.executable, str(script), "import-exemplars", args.path]
    if args.json:
        cmd.append("--json")
    result = subprocess.run(cmd)
    sys.exit(result.returncode)
...
def cmd_writer_plan(args):
    """Prepare a lightweight personal writing brief."""
    import subprocess
    script = Path(__file__).parent.parent / "scripts" / "personal_writer.py"
    cmd = [sys.executable, str(script), "plan", args.topic, "--format", args.format]
...
def cmd_writer_learn(args):
    """Learn author preferences from local markdown edits."""
    import subprocess
    script = Path(__file__).parent.parent / "scripts" / "personal_writer.py"
    cmd = [
        sys.executable,
...
def cmd_writer_qc(args):
    """Run lightweight QC checks for an article."""
    import subprocess
    script = Path(__file__).parent.parent / "scripts" / "personal_writer.py"
    cmd = [sys.executable, str(script), "qc", args.article, "--format", args.format]
    result = subprocess.run(cmd)
    sys.exit(result.returncode)


def cmd_writer_log(args):
    """Append a lightweight history record."""
    import subprocess
    script = Path(__file__).parent.parent / "scripts" / "personal_writer.py"
    cmd = [
        sys.executable,
...
这是一段正常的文章内容，用来展示不同主题的排版效果。rwrite 支持多种排版主题，每种都有独特的视觉风格。
...
function selectTheme(name) {{
  localStorage.setItem('rwrite-theme', name);
...
def main():
    ...
    # writer-import
    p_writer_import = sub.add_parser("writer-import", help="Import local markdown articles into the exemplar library")
    p_writer_import.add_argument("path", help="Markdown file or directory")
    p_writer_import.add_argument("--json", action="store_true", help="JSON output")

    # writer-plan
    p_writer_plan = sub.add_parser("writer-plan", help="Prepare a personal writing brief")
    p_writer_plan.add_argument("topic", help="Writing topic")
    p_writer_plan.add_argument("--keyword", action="append", default=[], help="Additional keyword")
    p_writer_plan.add_argument("-o", "--output", help="Optional plan output file")
    p_writer_plan.add_argument("--format", choices=["yaml", "json"], default="yaml", help="Output format")

    # writer-learn
    p_writer_learn = sub.add_parser("writer-learn", help="Learn author preferences from draft/final markdown")
    p_writer_learn.add_argument("--draft", required=True, help="Draft markdown path")
    p_writer_learn.add_argument("--final", required=True, help="Final markdown path")
    p_writer_learn.add_argument("--format", choices=["yaml", "json"], default="yaml", help="Output format")

    # writer-qc
    p_writer_qc = sub.add_parser("writer-qc", help="Run lightweight QC for a markdown article")
    p_writer_qc.add_argument("article", help="Markdown article path")
    p_writer_qc.add_argument("--format", choices=["yaml", "json"], default="yaml", help="Output format")

    # writer-log
    p_writer_log = sub.add_parser("writer-log", help="Append a lightweight history record")
    p_writer_log.add_argument("article", help="Markdown article path")
    p_writer_log.add_argument("--topic", required=True, help="Original topic")
    p_writer_log.add_argument("--plan", help="Plan file produced by writer-plan")
    p_writer_log.add_argument("--format", choices=["yaml", "json"], default="yaml", help="Output format")
    ...
    try:
        ...
        elif args.command == "writer-import":
            cmd_writer_import(args)
        elif args.command == "writer-plan":
            cmd_writer_plan(args)
        elif args.command == "writer-learn":
            cmd_writer_learn(args)
        elif args.command == "writer-qc":
            cmd_writer_qc(args)
        elif args.command == "writer-log":
            cmd_writer_log(args)
