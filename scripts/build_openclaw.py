...
EXEMPLAR_REL_DIR = Path("references") / "exemplars"
EXEMPLAR_KEEP_FILES = {".gitkeep"}
...
def transform_body(body: str) -> str:
    ...
    body = body.replace(
        "本文档中 `{baseDir}` 指本 SKILL.md 所在的目录（即 rwrite 的根目录）",
        "本文档中 `{baseDir}` 指本 SKILL.md 所在的目录（即 rwrite 的根目录）",
    )
...
def build(output_dir: Path):
    ...
    # Remove user-specific exemplar artifacts from exported builds.
    exported_exemplars = output_dir / EXEMPLAR_REL_DIR
    if exported_exemplars.exists():
        for item in exported_exemplars.iterdir():
            if item.name in EXEMPLAR_KEEP_FILES:
                continue
            if item.is_file():
                item.unlink()
        gitkeep = exported_exemplars / ".gitkeep"
...
def main():
    parser = argparse.ArgumentParser(description="Build OpenClaw-compatible rwrite skill")
    parser.add_argument(
        "-o", "--output",
